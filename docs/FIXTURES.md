...
## Request Fixtures (`tests/fixtures/requests/`)

Null-populated request fixture files for all Route `params_model` and `request_model` references.
Generated by `scripts/generate_request_fixtures.py`. These fixtures drive the fail-first pattern:
null values for required fields trigger `ValidationError` at test time, creating a visible backlog
of fixtures that need real "golden" data.

- **Total request fixtures**: 102
- **Null-populated (scaffold)**: 101
- **Pre-populated (real data)**: 1 (`CompanySearchRequest.json`)
- **Models with required fields (fail on null)**: 24 (validated by `test_request_fixtures.py`)
- **Models with all-Optional fields (pass on null)**: 78

Request fixtures are loaded by:
- `ExampleRunner._load_request_data()` — examples load fixture data at runtime
- `load_request_fixture()` / `load_request_kwargs()` — integration tests load and convert alias keys
